...
const defaultFields: FormField[] = [
  { type: 'name', label: 'Nome', required: false },
  { type: 'email', label: 'E-mail', required: false },
  { type: 'phone', label: 'Telefone', required: false },
  { type: 'message', label: 'Mensagem', required: false },
];

export const ContactFormSection = ({ userId }: ContactFormSectionProps) => {
  ...
  const exportSubmissions = () => {
    const csv = [
      ['Nome', 'E-mail', 'Telefone', 'Mensagem', 'Enviado em'].join(','),
      ...filteredSubmissions.map(s => 
        [
          ...
          s.phone || '',
          (s.message || '').replace(/,/g, ';'),
          format(new Date(s.submitted_at), 'dd/MM/yyyy HH:mm', { locale: ptBR })
        ].join(',')
      )
    ].join('\n');
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground max-w-2xl leading-relaxed">
              Um formulário de captura é uma poderosa ferramenta de marketing utilizada para coletar
              informações valiosas de potenciais clientes, conhecidos como "leads". Aqui você pode criar um
              formulário para capturar e exportar informações de clientes que acessarem seu perfil e se
              cadastrarem.
            </p>
          </div>
                ...
                <div className="space-y-6 py-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      ...
                      {fields.map((field, index) => (
                        <div
                          key={index}
                          className="grid grid-cols-[1fr,auto,auto] gap-2 items-center p-3 rounded-lg border border-border/50 hover:bg-accent/5 transition-colors"
                        >
                  ...
                  <div className="space-y-4 rounded-lg border border-border/50 p-4 bg-muted/20">
                    <div className="space-y-3">
                      ...
                      <RadioGroup
                        value={sendEmailNotifications ? 'yes' : 'no'}
                        onValueChange={(val) => setSendEmailNotifications(val === 'yes')}
                        className="space-y-2"
                      >
                    ...
                    <div className="space-y-3 pt-3 border-t border-border/50">
                      ...
                      <RadioGroup
                        value={requireFormFill ? 'yes' : 'no'}
                        onValueChange={(val) => setRequireFormFill(val === 'yes')}
                        className="space-y-2"
                      >
                        <div className="flex items-center space-x-2">
